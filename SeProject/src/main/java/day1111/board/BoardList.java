...
public class BoardList extends JPanel {
	...
	//게시물 가져오기
	//rs에 담겨진 데이터를 TableModel이 보유한 2차원배열 data에 대입
	public void getList() {
		...
		try {
			...
			//스크롤이 가능하고(next() 능력에 자유자재로 이동능력), 읽기전용 rs 만드는 옵션
			
			rs = pstmt.executeQuery();
			
			rs.last();
			int total = rs.getRow(); //총 레코드 수 반환			
			String[][] records = new String[total][5]; //2차원 배열 준비해 옮기기			
			rs.beforeFirst();
			...
			while(rs.next()) {
				...
				records[index++] = arr; //일차원 배열을 이차원 배열의 요소에 삽입				
			}
