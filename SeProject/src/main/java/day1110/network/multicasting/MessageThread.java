...
//이 Thread는 대화용 쓰레드 이므로 입출력 스트림을 각각의 인스턴스가 보유해야 한다.
//아래의 클래스를 쓰레드로 선언하는 순간부터, 이 인스턴스들은 서로 비동기적으로 동작 가
		...
		public void listen() {
			...
			try {
				while(flag) {
					...
					if(msg.equals("exit")) {
						//1) 벡터명단에서 제거    2)flag도 false: 쓰레드 Dead
						multiServer.clientList.remove(this); //나를 명단에서 제거						
						flag=false;
						multiServer.area.append("현재까지 접속자: "+multiServer.clientList.size()+"명\n");
					}else {
		...
		public void send(String msg) {
			try {
				for(int i=0; i<multiServer.clientList.size(); i++) {
					MessageThread messageThread =  multiServer.clientList.get(i);//벡터에 들어있는 메시지 쓰레드를 하나씩 꺼냄
